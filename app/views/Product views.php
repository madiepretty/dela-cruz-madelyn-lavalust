...
<head>
	...
	<title><?= !empty($form) ? 'Product Form' : 'Products' ?></title>
...
<body>
	<main>
		<?php if (!empty($form)): ?>
			<?php $editing = !empty($product['id']); ?>
			<h1><?= $editing ? 'Edit Product' : 'Add Product' ?></h1>
			<form method="post" action="<?= site_url($editing ? '/products/edit/' . (int) $product['id'] : '/products') ?>">
				<label>Name <input name="product_name" value="<?= htmlspecialchars($product['product_name'] ?? '', ENT_QUOTES, 'UTF-8') ?>" required></label>
				<label>Description <textarea name="description"><?= htmlspecialchars($product['description'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea></label>
				<label>Price <input type="number" name="price" min="0" step="0.01" value="<?= htmlspecialchars((string) ($product['price'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" required></label>
				<label>Quantity <input type="number" name="quantity" min="0" step="1" value="<?= htmlspecialchars((string) ($product['quantity'] ?? 0), ENT_QUOTES, 'UTF-8') ?>" required></label>
				<button type="submit">Save Product</button>
			</form>
			<p><a href="<?= site_url('/products') ?>">Back to products</a></p>
		<?php else: ?>
			<h1>Products</h1>
			<p>Signed in as <?= htmlspecialchars($username ?? '', ENT_QUOTES, 'UTF-8') ?> (<?= htmlspecialchars($role ?? 'user', ENT_QUOTES, 'UTF-8') ?>)</p>
			<p><?php if (!empty($is_admin)): ?><a href="<?= site_url('/products/create') ?>">Add Product</a> | <?php endif; ?><a href="<?= site_url('/logout') ?>">Log out</a></p>
			<table>
				<thead><tr><th>ID</th><th>Name</th><th>Description</th><th>Price</th><th>Quantity</th><?php if (!empty($is_admin)): ?><th>Actions</th><?php endif; ?></tr></thead>
				<tbody>
					...
					<tr>
						...
						<?php if (!empty($is_admin)): ?>
							<td>
								...
								<a href="<?= site_url('/products/delete/' . (int) $product['id']) ?>" onclick="return confirm('Delete this product?')">Delete</a>
							</td>
						<?php endif; ?>
					</tr>
				<?php endforeach; ?>
